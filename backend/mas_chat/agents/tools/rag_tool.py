...
# ------------------------------
@function_tool
def retrieve(query: str, top_k: int = 5, db_path: Optional[str] = None) -> str:
    """
    Retrieve the most relevant snippets from the local JSON DB.

    Args:
        query: User's question or search query.
        top_k: Number of top chunks to return.
        db_path: Optional custom DB file path (defaults to ./rag_demo_db.json)
    ...
    try:
        ...
        if not hits:
            return "No results found in the local knowledge base. Suggest ingesting a PDF."
